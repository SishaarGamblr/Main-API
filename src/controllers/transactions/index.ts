    ...
    async function create(
      ...
      await connection.transaction(async (tx) => {
        ...
        const [from, to] = await Promise.all([
          tx.findOneOrFail(Wallet, { where: { id: request.body.fromId }, lock: { mode: 'pessimistic_write' }}),
          tx.findOneOrFail(Wallet, { where: { id: request.body.toId }, lock: { mode: 'pessimistic_write' }})
        ]);
